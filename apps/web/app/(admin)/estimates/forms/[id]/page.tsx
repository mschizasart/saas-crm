...
export default function EstimateRequestFormEditorPage() {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const { show } = useToast();
  const id = params?.id;

  const [form, setForm] = useState<EstimateRequestForm | null>(null);
  ...
  const loadForm = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch(`/api/v1/estimate-request-forms/${id}`);
  ...
  const updateField = <K extends keyof EstimateRequestForm>(
    k: K,
    v: EstimateRequestForm[K],
  ) => {
  ...
  const publicUrl = useMemo(() => {
    ...
    return `${publicOrigin()}/request-quote/${orgSlug}/${form.slug}`;
  }, [form, orgSlug]);
  ...
  if (loading || !form) {
    return (
      <div className="max-w-6xl mx-auto">
        <PageHeader title="Edit form" backHref="/estimates/forms" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField label="Internal name" required>
                  <input
                    ...
                    placeholder="e.g. Construction quote requests"
                  />
                </FormField>
                ...
                <FormField label="Default client ID" hint="Optional — attach all requests to this existing client">
                  <input
                    className={inputClass}
                    value={form.defaultClientId ?? ''}
                    onChange={(e) => updateField('defaultClientId', e.target.value)}
                    placeholder="Leave empty to auto-create / match"
                  />
                </FormField>
                <FormField label="Default currency ID" hint="Optional — overrides the org default">
                  <input
                    className={inputClass}
                ...
                <FormField label="Assign to user ID" hint="Optional — suggested assignee, copied to the draft estimate's notes">
                  <input
                    className={inputClass}
          ...
          <Card>
            ...
            <CardBody>
              ...
              <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-3">
                Tip: include a long-text field with key <code>items</code>, <code>scope</code>, or <code>serviceType</code>{' '}
                — its value seeds the description on the placeholder line item of the draft estimate.
              </p>
            </CardBody>
          </Card>
