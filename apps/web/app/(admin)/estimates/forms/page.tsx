...
export default function EstimateRequestFormsListPage() {
  const router = useRouter();
  const { show } = useToast();

  const [forms, setForms] = useState<EstimateRequestForm[]>([]);
  ...
  const fetchForms = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch('/api/v1/estimate-request-forms');
  ...
  const handleDelete = async (form: EstimateRequestForm) => {
    if (!confirm(`Delete the form "${form.name}"? This cannot be undone.`)) return;
    try {
      const res = await apiFetch(`/api/v1/estimate-request-forms/${form.id}`, {
        method: 'DELETE',
      });
  ...
  const handleCopySnippet = async (form: EstimateRequestForm) => {
    const origin = publicOrigin();
    const snippet = `<iframe src="${origin}/request-quote/${orgSlug}/${form.slug}" width="100%" height="600" frameborder="0"></iframe>`;
    try {
  ...
  const handleCreate = async () => {
    ...
    const defaultSlug = `quote-${Math.random().toString(36).slice(2, 7)}`;
    try {
      const res = await apiFetch('/api/v1/estimate-request-forms', {
        method: 'POST',
        ...
        body: JSON.stringify({
          slug: defaultSlug,
          name: 'New quote request form',
          title: 'Request a quote',
          description: 'Tell us about your project and we will get back to you with a quote.',
          fields: [
            { key: 'name', label: 'Your name', type: 'text', required: true },
            { key: 'email', label: 'Email', type: 'email', required: true },
            { key: 'phone', label: 'Phone', type: 'phone', required: false },
            { key: 'company', label: 'Company', type: 'text', required: false },
            { key: 'items', label: 'What do you need?', type: 'textarea', required: true },
          ],
          captchaEnabled: true,
          isActive: true,
        }),
      ...
      const created: EstimateRequestForm = await res.json();
      router.push(`/estimates/forms/${created.id}`);
    } catch (err) {
  ...
  return (
    <ListPageLayout
      ...
      subtitle={
        !loading
          ? `${forms.length} form${forms.length === 1 ? '' : 's'} — submissions become draft estimates in your CRM`
          : undefined
      }
      primaryAction={{ label: 'New form', onClick: handleCreate, icon: <span className="text-lg leading-none">+</span> }}
      secondaryActions={[{ label: 'Back to estimates', href: '/estimates' }]}
    >
      {error && (
        <div className="mb-4">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title="No quote request forms yet"
          description="Create a form, embed it on your website, and every submission becomes a draft Estimate you can review."
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {forms.map((f) => {
                  const origin = publicOrigin();
                  const url = `${origin}/request-quote/${orgSlug}/${f.slug}`;
                  return (
                    <tr
                      key={f.id}
