...
const BULK_STATUS_OPTIONS: { value: EstimateStatus; label: string }[] = [
...
export default function EstimatesPage() {
  ...
  const confirmBulkStatus = async () => {
    ...
    setBulkLoading(true);
    try {
      const res = await apiFetch('/api/v1/estimates/bulk/status', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ estimateIds: Array.from(selected), status: pendingStatus }),
      });
      if (!res.ok) {
      ...
      if (json.skipped.length > 0) {
        ...
        toast.success(`Updated ${json.updated} estimate${json.updated !== 1 ? 's' : ''}`);
      }
      setSelected(new Set());
      ...
      secondaryActions={[
        {
          ...
          onClick: () =>
            void exportCsv(
              '/api/v1/estimates/export',
              `estimates-${new Date().toISOString().slice(0, 10)}.csv`,
              { entityLabel: 'estimates' },
            ),
        },
        { label: 'Pipeline view', href: '/estimates/pipeline' },
      ]}
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <TableSkeleton rows={6} columns={7} columnWidths={['16px', '40%', '50%', '30%', '30%', '25%', '20%']} />
              ) : items.length === 0 ? (
                <tr>
                ...
                <tr key={est.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50/60">
                  <td className="px-4 py-3">
                    <input
                      ...
                      aria-label={`Select estimate ${est.number}`}
                      className="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary/30"
                    />
                  </td>
      ...
      {pendingStatus && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="bulk-estimate-status-title"
            className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-md"
          >
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              <h2 id="bulk-estimate-status-title" className="text-base font-semibold text-gray-900 dark:text-gray-100">
                Change status
              </h2>
            </div>
            <div className="px-6 py-5 text-sm text-gray-700 dark:text-gray-300">
              Change {selected.size} selected estimate{selected.size !== 1 ? 's' : ''} to{' '}
              <span className="font-semibold">{capitalize(pendingStatus)}</span>?
              <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                Estimates already accepted or declined will be skipped unless moving to Expired.
              </p>
            </div>
