...
export default function NewCreditNotePage() {
  ...
  const [cnNumber, setCnNumber] = useState('0001');
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/v1/credit-notes?limit=1&page=1`, { headers })
      .then((r) => r.json())
      .then((d) => {
        const total = d.total ?? d.data?.length ?? 0;
        setCnNumber(String(total + 1).padStart(4, '0'));
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/v1/invoices?clientId=${clientId}&limit=100`, {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      router.push(`/credit-notes/${createdId}`);
    } catch (err) {
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-6 space-y-4">
            ...
            <Field label="Invoice (optional)">
              <select
                ...
                disabled={!clientId}
                className={inputClass + (clientId ? '' : ' bg-gray-50 cursor-not-allowed')}
              >
              ...
              {!clientId && (
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Select a customer first</p>
              )}
            </Field>
            ...
            <div>
              ...
              <div className="flex gap-1">
                <input value={cnPrefix} onChange={(e) => setCnPrefix(e.target.value)} className={inputClass + ' w-20 flex-shrink-0'} />
                <input value={cnNumber} onChange={(e) => setCnNumber(e.target.value)} className={inputClass} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Credit Note Date" required>
                <input type="date" required value={cnDate} onChange={(e) => setCnDate(e.target.value)} className={inputClass} />
              </Field>
        ...
        <div className="flex justify-end gap-3 pt-2">
          <Link href="/credit-notes" className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800">
            Cancel
          </Link>
          <button type="submit" disabled={saving || !clientId} className="px-5 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary/90 disabled:opacity-50 shadow-sm">
