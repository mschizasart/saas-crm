...
export default function ContractsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/v1/contracts?page=${page}&per_page=15`, { headers: { Authorization: `Bearer ${getToken()}` } });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/contracts/new" className="inline-flex items-center gap-1.5 bg-primary text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-primary/90">
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-12 text-center text-sm text-gray-400">No contracts yet</td></tr>
              ) : items.map((c) => (
                <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50/60">
                  ...
                  <td className="px-4 py-3 text-gray-600">{c.client?.company ?? c.client?.company_name ?? '—'}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/contracts/${c.id}`} className="text-xs text-gray-500 hover:text-primary font-medium">View</Link>
                  </td>
                </tr>
              ))}
            </tbody>
