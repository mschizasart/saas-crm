...
export default function NewInvoicePage() {
  ...
  useEffect(() => {
    ...
    // Fetch payment modes
    fetch(`${API_BASE}/api/v1/payments/modes`, { headers })
      .then((r) => r.json())
      .then((d) => setPaymentModes(Array.isArray(d) ? d : d.data ?? []))
      .catch(() => {});
    ...
    fetch(`${API_BASE}/api/v1/invoices?limit=1&page=1`, { headers })
      .then((r) => r.json())
      .then((d) => {
        const total = d.total ?? d.data?.length ?? 0;
        setInvoiceNumber(String(total + 1).padStart(4, '0'));
      })
      .catch(() => {});
  }, []);
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter((c) => {
      const name = (c.company ?? c.company_name ?? c.name ?? '').toLowerCase();
  ...
  function selectClient(client: ClientOption) {
    ...
    // Auto-fill addresses
    setBillStreet(client.billingStreet ?? '');
    setBillCity(client.billingCity ?? '');
    setBillState(client.billingState ?? '');
    setBillZip(client.billingZip ?? '');
    setBillCountry(client.billingCountry ?? '');
    setShipStreet(client.shippingStreet ?? '');
    setShipCity(client.shippingCity ?? '');
    setShipState(client.shippingState ?? '');
    setShipZip(client.shippingZip ?? '');
    setShipCountry(client.shippingCountry ?? '');

    // Fetch full client details for address fields
  ...
  const totals = useMemo(() => {
    ...
    for (const item of items) {
      ...
      // Tax 2
      if (item.taxId2) {
        const tax = taxes.find((t) => t.id === item.taxId2);
        if (tax) {
          const taxAmt = (lineTotal * Number(tax.rate)) / 100;
          if (!taxBreakdown[tax.id]) {
            taxBreakdown[tax.id] = { name: tax.name, rate: Number(tax.rate), amount: 0 };
  ...
  function togglePaymentMode(modeId: string) {
  ...
  async function handleSubmit(e: FormEvent, asDraft = false) {
    ...
    try {
      const payload: any = {
        ...
        number: `${invoicePrefix}${invoiceNumber}`,
        date: invoiceDate,
        ...
        allowedPaymentModes,
        isRecurring,
        recurringEvery: isRecurring ? Number(recurringEvery) || 1 : undefined,
        recurringType: isRecurring ? recurringType : undefined,
        totalCycles: isRecurring ? Number(totalCycles) || 0 : undefined,
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
            ...
            <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
              <input
                ...
                onChange={(e) => setPreventOverdueReminders(e.target.checked)}
                    ...
                    >
                      ...
                      <td className="py-3 pr-2 relative">
                        <input
                          ...
                          onBlur={() =>
                            setTimeout(() => {
                              if (activeAutocomplete === idx) setActiveAutocomplete(null);
                            }, 200)
                          }
                          onFocus={() => {
                            if (item.description.length >= 2) {
                              setActiveAutocomplete(idx);
                              searchSavedItems(item.description);
                            }
                          }}
                          className={inputClass}
                        ...
                        <button
                          ...
                          onClick={() => updateItem(idx, 'showLongDesc', !item.showLongDesc)}
                        >
                      ...
                      <td className="py-3 pr-2">
                        <select
                          value={item.taxId}
                          onChange={(e) => updateItem(idx, 'taxId', e.target.value)}
                          className={inputClass + ' mb-1'}
                        >
                        ...
                        <select
                          value={item.taxId2}
                          onChange={(e) => updateItem(idx, 'taxId2', e.target.value)}
  ...
  return (
    <div
      className={`flex justify-between ${
        bold ? 'font-bold text-gray-900 text-base' : 'text-gray-600'
      }`}
    >
      <span>{label}</span>
