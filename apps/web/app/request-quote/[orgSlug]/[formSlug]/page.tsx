...
// ---------------------------------------------------------------------------
// Public hosted "Request a quote" form — renders at
// /request-quote/{orgSlug}/{formSlug} and is intentionally kept outside every
// (admin) / (auth) / (portal) group so it stays accessible without a session.
// Submissions create draft Estimates that the tenant reviews and finalises.
// ---------------------------------------------------------------------------
...
export default function PublicEstimateRequestFormPage() {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/v1/public/estimate-request-forms/${encodeURIComponent(orgSlug!)}/${encodeURIComponent(formSlug!)}`,
          { cache: 'no-store' },
        );
        if (!res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/public/estimate-request-forms/${encodeURIComponent(orgSlug!)}/${encodeURIComponent(formSlug!)}/submit`,
        {
          method: 'POST',
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Your request has been received. We’ll review the details and follow up with a quote shortly.
              </p>
            </div>
          ) : (
